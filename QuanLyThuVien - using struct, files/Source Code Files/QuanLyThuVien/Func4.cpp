...
#pragma region Lap phieu tra sach
		...
		XoaDong();
		cout << "Nhap ma doc gia: " << MaDGCanTim << " - " << DocGia[ViTri_MaDG(DocGia, SoDocGia, MaDGCanTim)].HoVaTen;

		//In ra danh sach thong ke nhung sach chua tra cua doc gia
		cout << "\n\nDanh sach nhung sach chua tra cua doc gia " << MaDGCanTim << " - " << DocGia[ViTri_MaDG(DocGia, SoDocGia, MaDGCanTim)].HoVaTen << endl;
		...
		{
			if (strcmp(MuonTra[i].NgayTraTT, "chua tra ") != 0 && strcmp(MuonTra[i].NgayTraTT, "<mat sach>") != 0) continue;
			cout << " " << i << "."; //In ra ma code muon/tra sach
			cout << setw(9 - NumberLength(i)) << MuonTra[i].Ma; //In ra ISBN cua sach dang muon
			Spaces(5);
			cout << Sach[ViTri_MaSach(Sach, ListSach, MuonTra[i].Ma)].Ten; //In ra ten sach dang muon
			cout << setw(42 - strlen(Sach[ViTri_MaSach(Sach, ListSach, MuonTra[i].Ma)].Ten)) << MuonTra[i].NgayMuon; //In ra ngay muon sach
			cout << setw(16) << MuonTra[i].NgayTraDK; //In ra ngay tra sach du kien
			cout << endl;
		...
		cout << "\nNhap ma code (VD: 1): ";
		cin >> MaCode;
		while (!CheckMaCode(MaCode, ScanCode, ScanCode + MuonSoSach[ScanMaDG] - 1) || (strcmp(MuonTra[MaCode].NgayTraTT, "chua tra ") != 0 && strcmp(MuonTra[MaCode].NgayTraTT, "<mat sach>") != 0))
		{
			XoaDong();
		...
		if (strcmp(Func, "a") == 0)
		{
			...
			cin >> MuonTra[MaCode].NgayTraTT;
			while (!CheckNgayThangNam(MuonTra[MaCode].NgayTraTT) || KhoangCach2Ngay(MuonTra[MaCode].NgayMuon, MuonTra[MaCode].NgayTraTT) < 0 || KhoangCach2Ngay(MuonTra[MaCode].NgayTraTT, CurrentDate) < 0)
			{
				XoaDong();
				cout << "- Nhap ngay tra sach: ";
				cin >> MuonTra[MaCode].NgayTraTT;
			}
			LuuNgayThangNam(MuonTra[MaCode].NgayTraTT);
			XoaDong();
			cout << "- Nhap ngay tra sach: " << MuonTra[MaCode].NgayTraTT;
			Sach[ViTri_MaSach(Sach, ListSach, MuonTra[MaCode].Ma)].SoLuong++;
		}
		else
		{
			cout << "- Doc gia da lam mat sach";
			strcpy_s(MuonTra[MaCode].NgayTraTT, sizeof(MuonTra[100].NgayTraTT), "<mat sach>");
		}
		...
		if (strcmp(Func, "a") == 0)
		{
			if (KhoangCach2Ngay(MuonTra[MaCode].NgayMuon, MuonTra[MaCode].NgayTraTT) <= 7) cout << "- Sach duoc tra dung han\n";
			else
			{
				int ThoiGianTre = KhoangCach2Ngay(MuonTra[MaCode].NgayMuon, MuonTra[MaCode].NgayTraTT) - 7;
		...
		else
		{
			...
			int ViTri = ViTri_MaSach(Sach, ListSach, MuonTra[MaCode].Ma);

			//In ra tien phat
			int TienPhat = 2 * TienInt(Sach[ViTri].Gia);
			//cout << TienPhat;
			cout << "- Doc gia lam mat sach -> Nop phat: 200% x " << Sach[ViTri].Gia << " = " << TienString(TienPhat) << " VND" << endl;
		}
...
#pragma region In phieu muon sach
		...
		int ViTri = ViTri_MaSach(Sach, ListSach, MuonTra[MaCode].Ma);
		PhieuTraSach(MaDGCanTim, DocGia[ViTri_MaDG(DocGia, SoDocGia, MaDGCanTim)].HoVaTen, MuonTra[MaCode].Ma, Sach[ViTri_MaSach(Sach, ListSach, MuonTra[MaCode].Ma)].Ten, Sach[ViTri].Gia, MuonTra[MaCode].NgayMuon, MuonTra[MaCode].NgayTraDK, MuonTra[MaCode].NgayTraTT);
#pragma endregion
...
void PhieuTraSach(int MaDocGia, char HoVaTen[100], int MaSach, char TenSach[100], char GiaSach[100], char NgayMuon[100], char NgayTraDK[100], char NgayTraTT[100])
{
	...
	if (strcmp(NgayTraTT, "<mat sach>") != 0)
	{
		...
		if (KC - 7 > 0)
		{
			cout << "- Doc gia tra sach tre han " << KC - 7 << " ngay.\n";
