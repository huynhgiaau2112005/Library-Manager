...
void Func5b(int ListSach, char TheLoai[100][100], int SoSach[100])
{
	...
	for (int i = 1; i <= ListSach; i++)
	{
		...
		int SoSachTheoTheLoai = DemSoTrung(ListSach, TheLoai, SoSach, TheLoai[i], i, DemSoLuongSach); //Dem so sach co cung the loai
		if (SoSachTheoTheLoai != -1)
		{
			cout << TheLoai[i] << setw(25 - strlen(TheLoai[i])) << SoSachTheoTheLoai << setw(15 - NumberLength(SoSachTheoTheLoai)) << DemSoLuongSach << endl;
		}
		else continue;
	}
	for (int i = 1; i <= 46; i++) cout << "-";
...
int DemSoTrung(int ListSach, char TheLoai[100][100], int SoSach[100], char XetTheLoai[], int XetViTri, int& DemSoLuongSach)
{
	...
	{
		if (strcmp(upperString(TheLoai[i]), upperString(XetTheLoai)) == 0) return -1; //The loai nay trung voi the loai da duoc xet truoc do
	}
	int dem = 0;
	DemSoLuongSach = 0;
	for (int i = XetViTri; i <= ListSach; i++)
	{
		if (strcmp(upperString(TheLoai[i]), upperString(XetTheLoai)) == 0)
		{
			DemSoLuongSach += SoSach[i];
			dem++;
		}
	}
	return dem;
